...
namespace Invector
{
    public class v_AIController : v_AIAnimator
    {
        ...
        void ControlLocomotion()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    // turn on spot
                    if (agent.pathStatus != UnityEngine.AI.NavMeshPathStatus.PathInvalid && condition && !inAttack)
                    {
        ...
        protected virtual void UpdateDestination(Vector3 position)
        {
            ...
            if (agent.enabled && agent.isOnNavMesh && UnityEngine.AI.NavMesh.SamplePosition(position, out hit, 0.5f, groundLayer))
            {
